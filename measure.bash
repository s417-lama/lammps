    ...
    for thread in ${ANALYSIS_THREADS[@]}; do
      for size in ${SIZES[@]}; do
        for i in $(seq 1 $N_REPEATS); do
          ...
          numactl -iall mpirun ./lmp_kokkos_omp -in ../bench/in.lj -var x $size -var y $size -var z $size -k on t 56 -sf kk -pk kokkos newton off neigh full 2> $ERR_FILE | tee $OUT_FILE
        done
      done
    done
  done
    ...
    for thread in ${ANALYSIS_THREADS[@]}; do
      for size in ${SIZES[@]}; do
        for i in $(seq 1 $N_REPEATS); do
          ...
          numactl -iall mpirun ./lmp_kokkos_abt -in ../bench/in.lj -var x $size -var y $size -var z $size -k on t 56 -sf kk -pk kokkos newton off neigh full 2> $ERR_FILE | tee $OUT_FILE
        done
      done
    done
  done
